...
export function GetCreditsModal({ isOpen, onClose }: GetCreditsModalProps) {
  ...
  const handleCheckPaymentStatus = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${config.apiUrl}/api/oxapay/status/${paymentTrackId}`,
        {
          credentials: "include",
        }
      );
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-blue-700">
                        Pay with Bitcoin, Ethereum, USDT, or other
                        cryptocurrencies through our secure payment partner
                      </p>
                    </div>
                  </div>
                </div>
